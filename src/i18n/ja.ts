...
const lang = {
  ...
  mintPanel: {
    ...
    drawMessage:
      "ミントは、全部自分で描いた場合は無料ですが(画像の複雑度に応じたガス代は必要です)、リミックスやオーバーレイを活用した場合は{mintPrice}ETHが必要です。このミント代の97.5%はクリエーターに渡されますが、逆に、自分が描いた絵が他の人に利用された場合、それがあなたの収入になります（Draw2Earn）。",
    mintMessage1:
